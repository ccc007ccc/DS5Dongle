...
case "$TCM_PROFILE" in
    ...
    pvq-mdct-clusters)
        ...
        ;;
    pvq-mdct-clusters-pvq-data)
        TCM_DEFINES="-DM61_OPUS_TCM_PVQ_CLUSTER=1 -DM61_OPUS_TCM_MDCT_CLUSTER=1 -DM61_OPUS_TCM_PVQ_DATA=1"
        ;;
    pvq-mdct-decode-clusters)
        TCM_DEFINES="-DM61_OPUS_TCM_PVQ_CLUSTER=1 -DM61_OPUS_TCM_MDCT_CLUSTER=1 -DM61_OPUS_TCM_DECODE_DENORM=1 -DM61_OPUS_TCM_DECODE_SYNTH=1 -DM61_OPUS_TCM_DECODE_MDCT=1"
        ;;
    pvq-mdct-decode-mdct)
        TCM_DEFINES="-DM61_OPUS_TCM_PVQ_CLUSTER=1 -DM61_OPUS_TCM_MDCT_CLUSTER=1 -DM61_OPUS_TCM_DECODE_MDCT=1"
        ;;
...
case "$STAGE_PROFILE" in
    0)
        ...
        ;;
    1)
        STAGE_DEFINES="-DM61_OPUS_STAGE_PROFILE=1"
        ;;
    *)
        printf '[m61-opus-build] ERROR: stage profile must be 0 or 1\n' >&2
        exit 1
        ;;
esac

BUILD="$ROOT/build-${VARIANT}-${PATCH_PROFILE}-${TCM_PROFILE}-stage${STAGE_PROFILE}"
...
if [[ ! -f "$STAMP" || "$(cat "$STAMP")" != "$EXPECTED_STAMP" ]]; then
    ...
    mkdir -p "$BUILD"
    cd "$BUILD"
    CFLAGS="-${OPT} ${LTO_FLAGS} ${PATCH_DEFINES} ${TCM_DEFINES} ${STAGE_DEFINES} -g0 -ffunction-sections -fdata-sections -fno-common -fstrict-volatile-bitfields -march=rv32imafcp_zpn_zpsfoperand_xtheade -mabi=ilp32f -mtune=e907" \
        "$SOURCE/configure" \
