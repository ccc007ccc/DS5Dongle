...
Builds the M61 ESP32 programming bridge with Bouffalo SDK.
...
build_project() {
    ...
    local bin_file
    bin_file="$(find "$PROJECT_DIR/build/build_out" -maxdepth 1 -name 'm61_esp32_prog_bridge_*.bin' | head -1 || true)"
    if [[ -n "$bin_file" ]]; then
