...
### 7.8 decoder synthesis OCRAM 实验否决

阶段热点证明 finalize/anti-collapse/synthesis 约占 282k cycles 后，测试了两种 cached OCRAM
放置，正式默认 `pvq-mdct-clusters` 始终不变：

1. `denormalise_bands + celt_synthesis + clt_mdct_backward_c` 连续簇增加 1,712 B `.itcm`。
   目标阶段从 282,202 降到 226,409 cycles（-19.8%），但 decoder 总 cycles 从 877,363
   增到 890,082（+1.45%），encode 从 1,413,284 增到 1,491,420（+5.53%）。原因是插入
   `bands.o`/`celt_decoder.o` 后推移了已验证 PVQ 与 FFT/MDCT 热簇，造成全局 cache/layout
   回归。
2. 只放 `clt_mdct_backward_c`，增加 448 B `.itcm`，保持现有 PVQ/FFT 地址。目标阶段仍从
   282,202 降到 264,367 cycles（-6.32%），但 decoder 平均从 2,742 us / 877,363 cycles
   恶化到 2,884 us / 922,233 cycles（+5.1%），I-miss 从 1,830 增到 1,936。虽然 decoder
   max 从 4,644 us 降到 4,367 us，平均和 cache 门槛不通过，仍否决。

两轮所有 drop/deadline/cancel/stale/error 均为 0。该结果再次证明 cached OCRAM 只能按完整
