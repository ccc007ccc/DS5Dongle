...
BL616的HID IN端点使用项目内的完成事件适配层。原USB v2驱动在VDMA把数据装入FIFO3时
就报告传输完成，早于主机真正取走中断包。M61启用硬件FIFO3 IN事件，只在该事件后完成
EP4，从而及时为下一次USB轮询装填报告；共享SDK、FIFO布局、音频端点和64字节DualSense
报告长度均不改变。
