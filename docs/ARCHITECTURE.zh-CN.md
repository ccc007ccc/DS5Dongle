...
BL616的HID IN端点使用锁定SDK原生Full-Speed PHY/VDMA完成路径。项目不替换共享SDK的
USB中断处理、FIFO布局、音频端点或64字节DualSense报告长度。描述符间隔与最近样本调度
由M61配置选择；发布的250 Hz和500 Hz是主机侧硬件实测回报率，不代表蓝牙手柄以更高
频率采样。
...
| `m61_web_config.c` | 版本化WebHID配置、迁移和CRC记录 |
| `m61_stick_deadzone.c` | 左右独立缩放径向摇杆修正 |
...
公开管理面是Feature Report `0xF6`到`0xF9`上的版本化二进制WebHID协议。固件声明能力、
校验全部范围、异步执行命令，并负责带CRC保护的Flash记录。Shell命令只用于开发和恢复；
浏览器不会转发或解释shell文本。
