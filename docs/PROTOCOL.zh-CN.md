...
- `0xF7`：schema v5配置与能力位，包含左右独立的缩放径向摇杆死区、USB回报率模式、
  硬件PWM状态灯亮度和DualSense音频缓冲提示；
- `0xF8`：固件和产品身份；
- `0xF9`：telemetry v2连接、运行时、管理结果与受限健康计数；前8个payload字节
  保持兼容telemetry v1。

23-byte配置体以带CRC32的EasyFlash记录保存。记录无效时回退正式默认值：麦克风和
超频关闭、manual 320 MHz、扬声器启用并自动路由、自动重连和状态灯启用、触觉增益
1.0倍、空闲关机和主机挂起关机关闭、左右死区均为0%、状态灯亮度12%、音频缓冲提示
48，USB实时转发新蓝牙样本。v1到v4记录迁移时为新增字段采用上述发布默认值；已移除的
schema v4实验回报率仍迁移为固定500 Hz。
