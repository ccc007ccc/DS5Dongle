...
### 2026-07-13：E907位精确16x16 Q15 `khmbb`（否决）

- `MULT16_16_Q15`的debug契约要求输入和输出均为signed 16位，因此`khmbb`唯一的
  `INT16_MIN * INT16_MIN`饱和组合位于宏有效域外。边界组合、100万组随机输入、
  1200帧encoder逐字节输出和24万帧decoder checksum均通过位精确验证。
- 初版强制替换常量乘法使text增大1792 B；修正为保持32位返回类型、编译期常量继续
  使用generic后，最终ELF生成76条`khmbb`，普通`mul`从1907降至1781，text减少512 B。
- 真机首轮baseline-v1却从已提交Q15基线5287 us/1693169 cycles/226993 instret退化至
  5572 us/1784050/230627，分别恶化5.39%/5.37%/1.60%；P99从6250恶化至6750 us，
  I-cache miss从3406升至3645。该结果已明显越过单轮否决门槛，不再运行第二轮。
- `khmbb`的执行延迟和依赖链代价超过所减少的普通指令，且与早期PVQ局部实验方向一致。
  候选补丁、宏和测试入口均从默认源码移除；不得仅因text缩小重新启用。
