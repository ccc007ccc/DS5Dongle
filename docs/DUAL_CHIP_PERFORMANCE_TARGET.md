...
- `BT_CONNECT/BT_DISCONNECT` 可靠控制面已接入：M61 transport 和 `ds5 connect`/`ds5 disconnect [reconnect]` shell 命令会转发到 ESP32；双芯片 shell 现在区分 `ds5 autoconnect`（保存地址后再 fallback 扫描）、`ds5 scan`（强制扫描）和 `ds5 connect last`（只尝试保存地址）；ESP32 raw HIDP 也可按 6 字节 BDA 指定目标；如果显式 BDA connect 发生在 ESP32 `L2CAP/SDP` 尚未 ready 之前，该 target 会保留到 stack ready 后继续执行，不会被保存地址/扫描 fallback 覆盖；如果之后又收到泛化的 auto-connect 请求，也不会把这个 pending explicit target 覆盖掉；如果手动指定地址后的重连定时器触发，ESP32 也会保留这次手动 target，而不是退回保存地址/扫描路径；但一旦 HIDP control/interrupt 都成功打开，reconnect policy 会归一化回“按当前已保存地址重连”，避免一次性的 scan/manual 选择污染后续断线恢复。`HELLO` capability 现在会显式声明 mode-select `BT_CONNECT` 支持，避免新 M61 对旧 ESP32 固件默默发送不兼容的 1 字节 connect payload。`BT_DISCONNECT` 会清掉 pending target/discovery/SDP 状态，避免迟到的 completion 把连接又偷偷拉起。
- 保存地址重连现在不再猜测 ESP-IDF incoming L2CAP open 的 PSM；ESP32 同时监听 HID control/interrupt PSM，等待已配对手柄自己打开两条通道，5 秒内没形成完整 HIDP 就关闭 partial fd 并计一次 saved reconnect failure，自动模式达到阈值后再 fallback 扫描。
