...
同日继续测试了 band energy 小簇：在正式 PVQ+MDCT/FFT 簇后追加 `amp2Log2` 336 B 和
`compute_band_energies` 708 B，共新增 1044 B cached OCRAM 代码；`.itcm` 增至
28232 B，RAM 使用 197008 B（46.36%）。候选固件 SHA256 为
`72C40E38CE3C250318A4016F5220273B119E4F2DB4D3C255F675F5F436D6F8B7`。

测试期间一次外部供电中断产生的 46 次 deadline/cancel 已按前后差分排除。供电恢复后的
两轮有效 baseline-v1 结果为：

- 第一轮：4250 us / 1,362,301 cycles / 214,276 instret / 2937 I-cache miss；
- 第二轮：4271 us / 1,367,827 cycles / 215,481 instret / 2946 I-cache miss；
- 16877 帧加权平均：4261 us / 1,365,064 cycles / 214,879 instret /
  2942 I-cache miss；
- 相对正式 PVQ+MDCT/FFT 基线，时间慢约 7.31%，cycles 慢约 7.21%，instret 增加约
  1.77%，I-cache miss 增加约 12.86%；
- 两轮 queue drop、deadline、BT stale 和编码错误增量均为 0，但 P99 累计升至 5500 us。

因此否决 band energy 小簇迁移。该结果再次证明 cached OCRAM 本身不是充分条件；即使函数
每帧必经且总计只有约 1 KiB，把它们插入当前热代码布局仍会显著增加全路径冲突 miss。
`pvq-mdct-energy-clusters` 仅保留为可复现的测试配置，不作为正式默认值。

同日测试 `tf_analysis` 单独追加到正式 PVQ+MDCT/FFT 基线。该函数为 1464 B，LTO 将其
放在 `.itcm` 起始位置并整体平移后续 PVQ/MDCT 簇；`.itcm` 总量为 28648 B，RAM 使用
197424 B（46.46%）。候选固件 SHA256 为
`32F23B762859662CF52E24A28938E82D50797D4607AAC1A19C5D32694514B137`。

两轮 baseline-v1 各 8438 帧的结果为：

- 第一轮：4315 us / 1,384,434 cycles / 216,874 instret / 2857 I-cache miss；
- 第二轮：4225 us / 1,354,307 cycles / 214,343 instret / 2790 I-cache miss；
- 两轮平均：4270 us / 1,369,371 cycles / 215,609 instret / 2824 I-cache miss；
- 相对正式 PVQ+MDCT/FFT 基线，时间慢约 7.54%，cycles 慢约 7.55%，instret 增加约
  2.12%，I-cache miss 增加约 8.33%；
- 两轮实时错误均为 0，但累计 P99 为 5500 us，最大编码时间为 5990 us。

因此否决 TF 单函数迁移，`pvq-mdct-tf-clusters` 仅保留为可复现测试配置。energy 与 TF
连续两次失败说明当前正式布局已较敏感；停止继续按“小函数每帧必经”原则猜测 RAM 候选。
后续若再改变代码位置，必须先增加更细粒度的函数/调用簇 HPM 证据。真机测试不再额外运行
5 秒连接探测；直接执行正式 90 秒轮次，并通过该轮 `sent/stale` 差分判断连接有效性。
...
- 保留 D-cache preload=1、AMR=1；停止猜测式的小函数 RAM 迁移。下一步回到当前正式
  PVQ/MDCT/FFT 热簇内部，寻找位精确算术、循环和数据访问优化，或先补充更细粒度 HPM。
