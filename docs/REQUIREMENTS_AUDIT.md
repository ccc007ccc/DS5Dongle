...
- M61 原生 USB DualSense composite device 代码和构建配置。
- USB HID OUT `0x02` 到 BT `0x31` 的 `SetStateData` 状态缓存和 flag 合并。

仍缺少的完成证据：

- PC 通过 BL618 原生 `USB_DP/USB_DM` 枚举到 `VID_054C&PID_0CE6` DualSense 复合设备，这一项已实测成立。
- 蓝牙连接真实手柄后，`ds5 status` 显示 `usb_gamepad sent>0` 或持续增长。
...
4. PC 通过 BL618 原生 `USB_DP/USB_DM` 枚举到 DualSense composite，`ds5 status` 中 `usb_gamepad configured=1`。
...
`--strict-complete` 只有在日志和人工端到端证据都满足时才应该返回成功。当前 USB 枚举已经打通，但手柄输入映射、输出控制和稳定性仍需保留 pending 项。
