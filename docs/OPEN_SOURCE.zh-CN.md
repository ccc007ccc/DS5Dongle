...
本仓库只保留M61应用源码、构建元数据、测试和持续维护文档，不提交1 GiB以上的Bouffalo
SDK、编译器二进制、生成固件、基准日志或厂商PDF手册。Windows刷写器在CI构建时从锁定
SDK读取`BLFlashCommand.exe`并嵌入最终EXE，二进制本身不进入Git。
...
| `BLFlashCommand.exe` | 锁定SDK，刷写器构建时嵌入 | Bouffalo SDK Apache-2.0声明 |
...
- 源码commit/tag；
- 配套boot2、partition、应用BIN和校验清单；
- 包含上述四项的`M61-Firmware-<版本>.zip`；
- 需要时发布ELF、MAP；
- 生成的`.manifest.json`；
- MIT许可证和`THIRD_PARTY_NOTICES.md`；
- `M61-Flasher-Windows.exe`、其SHA256及Bouffalo Apache-2.0许可证；
- 说明精确硬件和profile的中英文release note。

不要发布：

- 复制进Git仓库的SDK/工具链二进制（Release刷写器按上述方式嵌入锁定刷写核心除外）；
- Opus build/cache目录；
- 私有手柄数据或配对密钥；
