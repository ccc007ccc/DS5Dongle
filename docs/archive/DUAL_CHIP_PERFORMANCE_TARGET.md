...
- `BT_CONNECT/BT_DISCONNECT` 可靠控制面已接入：M61 transport 和 `ds5 connect`/`ds5 disconnect [reconnect]` shell 命令会转发到 ESP32；双芯片 shell 现在区分 `ds5 autoconnect`（保存地址后再 fallback 扫描）、`ds5 scan`（强制扫描）和 `ds5 connect last`（只尝试保存地址）；ESP32 raw HIDP 也可按 6 字节 BDA 指定目标；如果显式 BDA connect 发生在 ESP32 `L2CAP/SDP` 尚未 ready 之前，该 target 会保留到 stack ready 后继续执行，不会被保存地址/扫描 fallback 覆盖；如果之后又收到泛化的 auto-connect 请求，也不会把这个 pending explicit target 覆盖掉；如果手动指定地址后的重连定时器触发，ESP32 也会保留这次手动 target，而不是退回保存地址/扫描路径；但只有 HIDP control/interrupt 都打开且收到可解析 `0x31` full report 后，reconnect policy 才会归一化回“按当前已验证地址重连”，避免一次性的 scan/manual 选择污染后续断线恢复。`HELLO` capability 现在会显式声明 mode-select `BT_CONNECT` 支持，避免新 M61 对旧 ESP32 固件默默发送不兼容的 1 字节 connect payload。`BT_DISCONNECT` 会清掉 pending target/discovery/SDP 状态，避免迟到的 completion 把连接又偷偷拉起。
- 保存地址重连现在不再猜测 ESP-IDF incoming L2CAP open 的 PSM；ESP32 同时监听 HID control/interrupt PSM，等待已配对手柄自己打开两条通道，并且只有收到 `0x31` full report 后才把 raw HIDP 置为 ready；5 秒内没形成完整 ready 状态或 bring-up 重试耗尽，就关闭 partial fd 并计一次 saved reconnect failure，自动模式达到阈值后再 fallback 扫描。
...
- ESP32 raw HIDP 现在会把“保存 DualSense 地址”和“成功显式重配后移除黑名单”的 NVS 持久化延后到收到有效 `0x31` full report 之后处理，而不是在 `L2CAP open` 热路径里同步刷 flash，减少刚连上时的阻塞风险并避免保存错误 peer。
- 可靠控制面的 ACK 与超时重传已接入：M61 对 `HELLO`、feature/reset 这类 reliable frame 带 `DS5_DUAL_FLAG_RELIABLE`；普通控制帧在 ESP32 解析完成后用同 type + `DS5_DUAL_FLAG_ACK` 回传原 seq/type/status，HIDP TX reliable 帧则保留原 seq 到 TX task 实际写入 DualSense 后再 ACK 成功或 `-ENOTCONN`/`-ETIMEDOUT` 等错误；M61 发送 reliable frame 后会 clock 空事务取 ACK，ACK miss 或可恢复错误会按 `CONFIG_M61_ESP32_RELIABLE_RETRY_COUNT` 重发，并在 transport stats 记录 ACK/miss/retry/fail/status。来自 ESP32 的负 ACK 只表示 peer/HIDP 状态，不触发 M61 reset recovery；`LATEST` 合并不会丢弃仍待 ACK 的 reliable 队列项，队列满时会让新 reliable 帧收到 `-ENOMEM`。
...
- ESP32 已能通过 `FLOW_CREDIT` 回报 BT TX 队列余量、raw HIDP ready（两条 HIDP 通道加有效 `0x31` full report）、SPI response drop 和 HIDP TX error 计数；M61 侧在 transport stats 中记录最近一次 credit，并会对 `DROP_OK` 的最新实时包做主动限流。
