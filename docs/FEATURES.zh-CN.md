...
| 板载UI | 状态灯亮度持久化 | 已通过构建验证 | 1–100%，默认12%；直接更新BL616硬件PWM，不增加周期性软件调光任务 |
| 音频 | DualSense缓冲提示持久化 | 已通过构建验证 | 16–127，默认48；只修改手柄音频报告提示，不额外分配MCU缓冲区 |
| 恢复 | 双语Windows GUI刷写器 | 已完成无板验证 | 在线/本地校验固件ZIP、CH340检测/驱动修复、460800/115200 baud、可复制日志；手动BOOT/RESET仍是可靠ISP路径 |
